    ...
    for ii in range(len(beam_lst)):
        ...
        if int(rank) == 0:
            ...
            for idx_j, (a,b,j) in enumerate(states_span):
                ...
                # true state  with meaning, define  S.
                ...
                for ee in ref:
                    ...
                    for word in pred:
                        if word in ref_word:
                            score_ += 1
                            match_1 += 1
                    match_temp.append(match_1)
                    full_temp.append(len(ref_word))

                    final_score_ = score_ / (len(pred))
                    # print(final_score_)
                    start_.append(final_score_)

                match_temp = np.array(match_temp)
                full_temp = np.array(full_temp)
                index_temp = np.argmax(match_temp / full_temp)

                match_ += match_temp[index_temp]
                denom_recall +=  full_temp[index_temp]

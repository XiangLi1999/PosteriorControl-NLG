...
class RNNLM2(nn.Module):
    def __init__(self, options):
        ...
        self.table_dim = options.table_dim
        self.conditional_dim = options.table_hidden_dim * 2
        self.table_hidden_dim = options.table_hidden_dim
        self.vocab_size = options.vocab_size
        self.field_vocab_size = options.field_vocab_size
        self.idx_vocab_size = options.idx_vocab_size
        self.gen_vocab_size = options.gen_size
        self.i_dim = options.i_dim
        self.f_dim = options.f_dim
        ...
        # inp_feats = 4
        # if self.mlpinp:
        #     mlpinp_sz = inp_feats * self.w_dim
        #     rnninsz = self.w_dim
        #     self.inpmlp = nn.Sequential(nn.Linear(mlpinp_sz, rnninsz),
        #                                 nn.ReLU(), nn.Linear(rnninsz, rnninsz))
        # else:
        #     rnninsz = self.w_dim


        self.word_vecs = nn.Embedding(self.vocab_size, options.embedding_dim, padding_idx=options.pad_idx)
        # self.field_vecs = nn.Embedding(self.tagset_size, options.f_dim, padding_idx=options.pad_idx)
        self.field_vecs = nn.Embedding(self.field_vocab_size, options.f_dim, padding_idx=options.pad_idx)
        self.idx_vecs = nn.Embedding(self.idx_vocab_size, options.i_dim, padding_idx=options.pad_idx)
        self.table_lstm = nn.LSTM(self.table_dim, self.table_hidden_dim, batch_first=True, bidirectional=True)



        ##1
        # if word_embeds is None:
        #     self.word_vecs = nn.Embedding(self.vocab_size, self.w_dim, padding_idx=options.pad_idx)
        #     self.max_pool = options.max_pool
        #     self.src_bias = nn.Parameter(torch.randn(1, self.w_dim))
        #     self.uniq_bias = nn.Parameter(torch.randn(1, self.w_dim))
        # else:
        #     self.word_vecs = word_embeds
        ...
        if options.option == 'beam':
            print('--'*30, 'changing to cpu')
            self.word_vecs.cpu()
            self.field_vecs.cpu()
            self.state_vecs.cpu()

        '''attention and copy attention'''
        self.h0_lin = nn.Linear(self.conditional_dim, 2 * self.h_dim)
        ...
        self.state_pick = nn.Sequential(nn.Linear(attn_dim, attn_dim // 2), nn.ReLU(),
                                        nn.Linear(attn_dim // 2, self.tagset_size - 1))
        self.vocab_pick = nn.Sequential(nn.Linear(attn_dim, attn_dim), nn.ReLU(),
                                        nn.Linear(attn_dim, self.gen_vocab_size + 1))


        self.bos_emb = nn.Parameter(torch.zeros(1, 1, rnninsz + self.s_dim))

        # self.bos_state = torch.LongTensor(1, 1).fill_(self.tagset_size - 1).to(options.device)
        ...
        # if self.full_independence == 1:
        #     self.rnn = nn.LSTM(rnninsz + self.s_dim, self.h_dim, num_layers=self.num_layers,
        #                        batch_first=True, dropout=options.dropout)
        #     self.forward = self.forward_fullrnn
        #     self.beam_forward = self.beam_forward_fullrnn
        #     self.start_vecs = nn.Parameter(torch.zeros(1, 1, self.h_dim))
        #     self.state_pick = nn.Linear(attn_dim, self.tagset_size-1)
        #     self.vocab_pick = nn.Linear(attn_dim, self.gen_vocab_size+1)
        #     self.action_pick = nn.Linear(attn_dim, 1)
        #     self.bos_emb = nn.Parameter(torch.zeros(1, 1, rnninsz + self.s_dim))
        #     self.bos_state = torch.LongTensor(1, 1).fill_(self.tagset_size - 1).to(options.device)
        # elif self.full_independence == 0:
        #     self.rnn = nn.LSTM(rnninsz + self.s_dim, self.h_dim, num_layers=self.num_layers,
        #                        batch_first=True,  dropout=options.dropout)
        #     # the split function
        #     self.bos_state = torch.LongTensor(1, 1).fill_(self.tagset_size - 1).to(options.device)
        #
        #     self.prior = options.prior
        #
        #     if self.prior == 'markov':
        #         self.calc_prior = self.calc_markov_prior
        #     elif self.prior == 'ar':
        #         self.calc_prior = self.calc_ar_prior
        #         self.ar_hidden = 50
        #         self.ar_lin = nn.Linear(self.conditional_dim, 2 * self.ar_hidden)
        #         self.ar_rnn = nn.LSTM(self.s_dim, self.ar_hidden, num_layers=1, batch_first=True)
        #         self.ar_state = nn.Linear(self.ar_hidden, self.tagset_size)
        #         self.bos_state_emb = nn.Parameter(torch.zeros(1, 1, self.s_dim))
        #
        #     else:
        #         self.calc_prior = self.calc_vamp_prior
        #
        #     self.forward = self.prior_recon
        #     self.bos_emb = nn.Parameter(torch.zeros(1, 1, rnninsz))
        #     self.vocab_pick = nn.Linear(attn_dim, self.gen_vocab_size + 1)
        #     self.Kmul = 1
        ...
        #
        #     self.cond_A_dim = options.cond_A_dim
        #     self.smaller_cond_dim = self.tagset_size * self.Kmul * self.cond_A_dim
        #     ###############   transition scores  ##############
        #     self.init_lin = nn.Linear(self.w_dim, self.tagset_size * self.Kmul)
        ...
        #         nn.Linear(self.w_dim, self.smaller_cond_dim),
        #         nn.ReLU(),
        #         nn.Linear(self.smaller_cond_dim, self.tagset_size * self.Kmul * self.cond_A_dim * 2))
        #     self.L = options.L
        #     self.L_from = nn.Parameter(torch.randn(self.tagset_size * self.Kmul, self.cond_A_dim))
        #     self.L_to = nn.Parameter(torch.randn(self.cond_A_dim, self.L))
        #     # len_lps = torch.mm(self.L_from, self.L_to).unsqueeze(0).expand(bsz, K, self.L)
        #
        # # similar to the RNNG idea with compose.
        # elif self.full_independence == 2:
        #     attn_dim = self.h_dim + self.conditional_dim
        #     self.rnn = nn.LSTM(self.w_dim, self.h_dim, num_layers=self.num_layers,
        #                        batch_first=True,  dropout=options.dropout)
        #     self.forward = self.forward_compose
        #     self.beam_forward = self.beam_forward_compose
        #     self.start_vecs = nn.Parameter(torch.zeros(1, 1, self.h_dim))
        #     self.state_pick = nn.Sequential(nn.Linear(attn_dim, attn_dim//2),nn.ReLU(), nn.Linear(attn_dim//2, self.tagset_size-1))
        #     self.vocab_pick = nn.Sequential(nn.Linear(attn_dim, attn_dim),nn.ReLU(), nn.Linear(attn_dim, self.gen_vocab_size+1))
        #     self.action_pick = nn.Sequential(nn.Linear(self.h_dim, self.h_dim//2),nn.ReLU(), nn.Linear(self.h_dim//2, 1))
        #     self.bos_emb = nn.Parameter(torch.zeros(1, 1, self.w_dim))
        #     self.bos_state = torch.LongTensor(1, 1).fill_(self.tagset_size - 1).to(options.device)
        #     self.bos_emb_states = nn.Parameter(torch.randn(self.tagset_size, 1, 1, self.w_dim))
        #     # nn.Sequential(nn.LSTM(self.w_dim, self.h_dim, num_layers=self.num_layers, batch_first=True,
        #     #                       bidirectional=True), nn.Linear(2*self.h_dim, self.h_dim))
        #     self.rnn_compose =  nn.LSTM(self.w_dim, self.h_dim, num_layers=self.num_layers, batch_first=True)
        #
        # elif self.full_independence == 3:
        #     self.rnn = nn.LSTM(rnninsz + self.s_dim, self.h_dim, num_layers=self.num_layers,
        #                        batch_first=True,  dropout=options.dropout)
        #     self.forward = self.forward_easy
        #     self.beam_forward = self.beam_forward_easy
        ...
        #     self.state_out_biases2 = nn.Parameter(torch.randn(1, 1, out_hid_sz))
            ##1
            # self.state_att_gates2 = nn.Parameter(torch.randn(1, 1, self.h_dim))
            # self.state_att_biases2 = nn.Parameter(torch.randn(1, 1, self.h_dim))
            # if self.sep_attn:
            #     self.state_att2_gates2 = nn.Parameter(torch.randn(1, 1, self.h_dim))
            #     self.state_att2_biases2 = nn.Parameter(torch.randn(1, 1, self.h_dim))
            # self.state_out_gates2 = nn.Parameter(torch.randn(1, 1, out_hid_sz))
            # self.state_out_biases2 = nn.Parameter(torch.randn(1, 1, out_hid_sz))
    ...
    def get_state_seq2(self, sample_lst, seqlen):
        sample_size, bsz, seqlen, _, _, _ = sample_lst.shape
        temp = sample_lst.nonzero()
        state = temp[:, -1]
        length = temp[:, 3]
        result = torch.repeat_interleave(state, length, dim=0).reshape(sample_size, bsz, seqlen)
    ...
    def forward_easy(self, condi, sample_lst):
        ...
        targs = condi['targs']
        targs = targs.view(bsz_orig, seqlen, maxlocs)
        srcenc = condi["srcenc"]
        ...
        h, (h_prev, c_prev) = self.rnn(rnn_inp, (h_prev, c_prev))  # h.shape = (batch_size * sample_size, seqlen, h_dim)
        h = self.dropout(h)
        h1, copy_attn = self.get_attn(h[:, :-1], state_names, srcfieldenc, fieldmask)
        ...
        # print(targs.shape)
        # print(targs[0].view(-1))
        # print(wlps_k.shape)
        # print(copy_attn.shape)
        # print(self.vocab_pick)
        logp_word_temp = torch.gather(wlps_k, 2, targs)

        # print(logp_word_temp.shape)
        # print(logp_word_temp.shape)
        logp_word = torch.sum(logsumexp2(logp_word_temp), dim=-1)


        # pick state
        h2 = self.get_attn2(h[:,:-1], srcfieldenc, fieldmask)
    ...
    def beam_forward_easy(self, condi, bsz, time=None):
        beam_width = 5
        beam_w_ = 1
                ...
                for elem in range(min(len(prev_nodes), beam_width)):
                    ...
                    for new_k in range(beam_w_):
                        ...
                        for new_k_w in range(beam_width):
                            ...
                            # rnninp
                            temp_word_emb = self.get_single_embs(decoded_feat.to(self.device)).view(1, 1, -1)
                            temp_state_emb = self.state_vecs(decoded_ts).view(1, 1, -1)
                            rnninp = torch.cat([temp_word_emb, temp_state_emb], dim=-1)
        ...
        for idx in range(bsz):
            ...
            while True:
                ...
                for new_k in range(beam_w_):
                    ...
                    out_rnn, copy = self.attn_step(h, decoded_ts.item(), srcfieldenc[idx:idx+1], fieldmask[idx:idx+1])
                    temp_wordlp = nn.LogSoftmax(dim=-1)(torch.cat([self.vocab_pick(out_rnn), copy], dim=-1))
                    log_prob_w, indexes_w = torch.topk(temp_wordlp, beam_width + window_size)
                    ...
                    for new_k_w in range(beam_width):
                        ...
                        if decoded_tw > self.gen_vocab_size:
                            # generate out of gen_vocab: copy
                            temp_word_id = decoded_tw - self.gen_vocab_size - 1
                            decoded_feat = torch.LongTensor(src_wrd2fields[idx][temp_word_id]).to(self.device)  # bsz x seqlen x 4
                            decoded_tw = decoded_feat[0]
                        else:
                            # generate from the gen vocab.
                            temp = self.temp_field
    ...
    def attn_step(self, h, k, srcfieldenc, fieldmask, mod='vocab'):
        ...
        if mod == 'vocab':
            ...
            srcfieldenc_ = F.tanh(self.attn_src(srcfieldenc).transpose(1, 2))  # (bsz, src_field_len, w_dim) -> (bsz, src_field_len, m_dim)
    ...
    def get_attn(self, h, k, srcfieldenc, fieldmask):
        ...
        bsz, seglen, cond_dim = h.shape
        mid_dim = self.mid_dim
        state_att_gates = torch.index_select(self.state_att_gates, 0, k.view(-1)).view(bsz*seglen, cond_dim, mid_dim) # (bsz*sample, seglen, h_dim)
        state_att_biases = torch.index_select(self.state_att_biases, 0, k.view(-1)).view(bsz*seglen, 1, mid_dim) # (bsz*sample, seglen, h_dim)
        attnin1 = torch.bmm(h.contiguous().view(bsz * seglen, 1, cond_dim), state_att_gates) + state_att_biases # (bsz*sample, seglen, h_dim)
        attnin1 = F.tanh(attnin1).view(bsz, seglen, mid_dim) # (bsz*sample, seglen, h_dim)

        srcfieldenc_ = F.tanh(self.attn_src(srcfieldenc)) #(bsz, src_field_len, w_dim) -> (bsz, src_field_len, m_dim)
        srcfieldenc_ = torch.repeat_interleave(srcfieldenc_, self.sample_size, dim=0).transpose(1, 2) # (bsz*sample_size, src_field_len, m_dim)
        ...
        if self.sep_attn:
            state_att2_gates = torch.index_select(self.state_att2_gates, 0, k.view(-1)).view(bsz*seglen, cond_dim, mid_dim)  # (bsz*sample, seglen, h_dim)
            state_att2_biases = torch.index_select(self.state_att2_biases, 0, k.view(-1)).view(bsz*seglen, 1, mid_dim)  # (bsz*sample, seglen, h_dim)
            attnin2 = torch.bmm(h.contiguous().view(bsz * seglen, 1, cond_dim), state_att2_gates) + state_att2_biases  # (bsz*sample, seglen, h_dim)
    ...
    def forward_easy_dual(self, condi, sample_lst):
        ...
        # word_embs = self.word_vecs(x)
        # word_embs = self.dropout(word_embs)
        # inpembs = self.inpmlp(word_embs.view(seqlen, bsz_orig, maxlocs, -1)).mean(2d)
        ...
        h = self.dropout(h)
        state_names = state_names.contiguous()
    ...
    def get_attn2_dual(self, h, srcfieldenc, fieldmask, field_vecs):
        ...
        state_att_biases_dual = self.state_att_biases2_dual.expand(bsz * seglen, 1, mid_dim)  # (bsz*sample, seglen, h_dim)
        ...
        srcfieldenc_dual = F.tanh(self.attn_src2_dual(field_vecs))  # (bsz, src_field_len, w_dim) -> (bsz, src_field_len, m_dim)
    ...
    def get_attn_dual(self, h, k, srcfieldenc, fieldmask, field_vecs):
        ...
        # the beta scores.
        state_att_gates_dual = torch.index_select(self.state_att_gates_dual, 0, k.view(-1)).view(bsz * seglen, cond_dim, mid_dim)
        state_att_biases_dual = torch.index_select(self.state_att_biases_dual, 0, k.view(-1)).view(bsz * seglen, 1, mid_dim)
        attnin1_dual = torch.bmm(h.contiguous().view(bsz * seglen, 1, cond_dim),
                            state_att_gates_dual) + state_att_biases_dual  # (bsz*sample, seglen, h_dim)
        attnin1_dual = F.tanh(attnin1_dual).view(bsz, seglen, mid_dim)  # (bsz*sample, seglen, h_dim)
        srcfieldenc_dual = F.tanh(self.attn_src_dual(field_vecs))  # (bsz, src_field_len, w_dim) -> (bsz, src_field_len, m_dim)
    ...
    def beam_forward_easy_dual(self, condi, bsz, timing=None):
        ...
        for idx in range(bsz):
            ...
            for i in range(max_len):
                ...
                for elem in range(min(len(prev_nodes), beam_width)):
                    ...
                    for new_k in range(beam_w_):
                        ...
                        ''' this section should be moved to CPU. '''
                        for elem1, elem2 in zip(indexes_w.cpu().view(-1), log_prob_w.cpu().view(-1)):
                            if elem1.item() in [x[0] for x in n_top['window'][-window_size:]] or elem1.item() == self.unk:
                                continue
                            else:
                                temp_ww.append(elem1)
                                temp_pp.append(elem2)
                        ...
                        for new_k_w in range(beam_width):
                            ...
                            if node['wordid'] == self.eos_idx and node['prevNode'] != None:
                                endnodes.append((score, node))
                            else:
                                nextnodes.append((score, node))
            ...
            endnodes = endnodes + next_nodes

            end_temp  = []
            for score, nn_ in endnodes:
                length_pen = ((nn_['leng'] + 5) / (5 + 1)) ** 1
                end_temp.append((score / length_pen, nn_))
    ...
    def beam_forward_easy_dua_noln(self, condi, bsz, timing=None):
        beam_width = 3
        beam_w_ = 2
        topk = 1  # how many sentence do you want to generate
        decoded_batch = []
        ...
        for idx in range(bsz):
            ...
            # node = BeamSearchNode(decoder_hidden, None, self.bos_idx, 0, 1, -1, -1, rnninp=self.bos_emb, window=[])
            prev_nodes = []
            prev_nodes.append((-node['logp'] / node['leng'], node))
            # start beam search
            break_flag = False
            while True:
                # give up when we have enough good results.
    ...
    def beam_forward_easy_dual4(self, condi, bsz, timing=None):
        ...
        for idx in range(bsz):
            ...
            while True:
                ...
                out_rnn, copy = self.attn_step_dual(torch.repeat_interleave(h, beam_w_, dim=0), indexes, srcfieldenc[idx:idx + 1],
                                               fieldmask[idx:idx + 1], srcfieldenc_w, srcfieldenc_dual_w)
    ...
    def attn_step_dual4(self, h, k, srcfieldenc, fieldmask, srcfieldenc_, srcfieldenc_dual, mod='vocab'):
        ...
        if mod == 'vocab':
            ...
            attnin1 = torch.bmm(h.contiguous().view(bsz*seglen, 1, cond_dim),
                                state_att_gates) + state_att_biases  # (bsz*sample, seglen, h_dim)
            attnin1 = F.tanh(attnin1).view(bsz, seglen, mid_dim)  # (bsz*sample, seglen, h_dim)
            srcfieldenc_ = srcfieldenc_.transpose(1, 2).expand(bsz, -1, -1)  # (bsz, src_field_len, w_dim) -> (bsz, src_field_len, m_dim)
            ascores = torch.bmm(attnin1, srcfieldenc_)  # (bsz*sample_size, seglen,  src_field_len)
            fieldmask = fieldmask.unsqueeze(1).expand_as(ascores)
            ascores[fieldmask] = neginf  # (bsz*sample_size, seglen,  src_field_len)
            aprobs = F.softmax(ascores, dim=2)  # (bsz*sample_size, seglen,  src_field_len)
            ...
            attnin1_dual = torch.bmm(h.contiguous().view(bsz*seglen, 1, cond_dim),state_att_gates_dual) + state_att_biases_dual
            attnin1_dual = F.tanh(attnin1_dual).view(bsz, seglen, mid_dim)  # (bsz*sample, seglen, h_dim)
            ...
            if self.sep_attn:
                ...
                state_att2_gates_dual = torch.index_select(self.state_att2_gates_dual, 0, k.view(-1)).view(bsz * seglen,
                                                                                                         cond_dim,
                                                                                                         mid_dim)
                state_att2_biases_dual = torch.index_select(self.state_att2_biases_dual, 0, k.view(-1)).view(bsz * seglen,
                                                                                                           1, mid_dim)
            ...
            # srcfieldenc_ = srcfieldenc_  # (bsz, src_field_len, w_dim) -> (bsz, src_field_len, m_dim)
            ascores = torch.bmm(attnin1, srcfieldenc_.transpose(1, 2).expand(bsz, -1, -1))  # (bsz*sample_size, seglen,  src_field_len)

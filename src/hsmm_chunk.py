...
#         print('uniform transition matrix:{}'.format(self.trans_unif))
#         if not self.trans_unif:
#             self.get_trans = self.get_transition_score
#             self.init_lin = nn.Linear(self.embedding_dim, self.tagset_size * self.Kmul)
...
#                 nn.Linear(self.embedding_dim, self.smaller_cond_dim),
#                 nn.ReLU(),
...
#         if args.decoder == 'gen':
#             out_hid_sz = self.hidden_dim
#             self.decoder = nn.Linear(out_hid_sz,
#                                      self.vocab_size + 1)  # the last element in this set is the end of phrase index.
#             self.zeros = torch.Tensor(1, 1).fill_(neginf).to(args.device)
#             self.eop_idx = self.vocab_size
#             self.one_rnn = args.one_rnn
#             self.seg_rnns = nn.ModuleList()
#             if args.one_rnn:
#                 print('not here lisa is not here ')
...
#             else:
#
#                 print('here lisa is here')
#                 for _ in range(args.tagset_size):
#                     self.seg_rnns.append(
#                         nn.LSTM(self.embedding_dim, self.hidden_dim, self.layers, dropout=args.dropout))
#             self.emb_drop = True
...
#                                                   nn.Tanh(), self.dropout,
#                                                   nn.Linear(self.hidden_dim * 2, self.tagset_size))
#
...
#                 pass
#             elif args.pr_reg_style == 'toy':
#                 self.posterior_reg_term = self.posterior_reg_term_toy
#             else:
...
#             pass
#
#     def make_combo_tags(self, sent, L=5):
#         bsz, seqlen = sent.shape
#         result = torch.zeros(L, bsz, seqlen).to(self.device)
#         for i in range(L):
#             if i > 0:
#                 result[i] = torch.cat([sent[:, i:], torch.ones(bsz, i).long().to(self.device)], dim=-1)
#             else:
#                 result[i] = sent[:, i:]
#         return result
#
#     def get_sample(self, condi, sample_size):
#         pass
#
#     def get_entr(self, example):
#         pass


class HSMM_chunk(nn.Module):
    def __init__(self, args):
        '''
        initialization step.
        '''
        # embedding_dim, hidden_dim, vocab_size, tag_set_size,  q_dim=20, dropout=0,
        super(HSMM_chunk, self).__init__()
        ...
        if not self.trans_unif:
            self.get_trans = self.get_transition_score
            self.init_lin = nn.Parameter(torch.randn(self.tagset_size * self.Kmul))
            # self.init_lin = nn.Linear(self.embedding_dim, self.tagset_size * self.Kmul)
            self.A_from = nn.Parameter(torch.randn(self.tagset_size * self.Kmul, self.cond_A_dim))
            self.A_to = nn.Parameter(torch.randn(self.cond_A_dim, self.tagset_size * self.Kmul))
            # self.cond_trans_lin = nn.Sequential(
        ...
        if args.decoder == 'gen':
            ...
            self.decoder = nn.Linear(out_hid_sz, self.vocab_size + 1) # the last element in this set is the end of phrase index.
            self.zeros = torch.Tensor(1, 1).fill_(neginf).to(args.device)
            self.eop_idx = self.vocab_size
            self.one_rnn = args.one_rnn
            self.seg_rnns = nn.ModuleList()
        ...
        elif args.decoder == 'gen1':
            self.s_dim = args.s_dim
            self.emission_lookup = nn.Sequential(nn.Linear(self.s_dim, self.embedding_dim), nn.Tanh(),
                                                 self.dropout, nn.Linear(self.embedding_dim, self.vocab_size),
                                                 nn.LogSoftmax(dim=1))

        elif args.decoder == 'gen2':
            self.s_dim = args.s_dim
            self.emission_linear = nn.Sequential(nn.Linear(self.s_dim, self.embedding_dim), nn.Tanh(), self.dropout,
                                                 nn.Linear(self.embedding_dim, self.embedding_dim))


        elif args.decoder == 'crf-gen':
            # self.h0_lin = nn.Linear(self.embedding_dim, 2 * self.hidden_dim * self.layers)
            # self.state_att_gates = nn.Parameter(torch.randn(self.tagset_size, 1, 1, self.hidden_dim))
            # self.state_att_biases = nn.Parameter(torch.randn(self.tagset_size, 1, 1, self.hidden_dim))
            # self.sep_attn = args.sep_attn
            ...
            self.seg_rnns.append(nn.LSTM(self.embedding_dim, self.hidden_dim, self.layers, dropout=args.dropout))
            self.state_embs = nn.Parameter(torch.randn(self.tagset_size, 1, 1, self.embedding_dim))
            # self.q_binary = nn.Sequential(nn.Linear(self.hidden_dim, self.hidden_dim),
            #                                       nn.Tanh(), self.dropout, nn.Linear(self.hidden_dim, self.tagset_size))
            self.q_binary =  nn.Sequential(nn.Linear(self.hidden_dim, self.tagset_size), nn.LogSoftmax(dim=-1))
        ...
        if args.posterior_reg == 1:
            if args.pr_reg_style == 'swap':
                ...
                self.posterior_reg_term = self.posterior_reg_term_toy
            elif args.pr_reg_style == 'ptb1':
                self.posterior_reg_term = self.posterior_reg_term_ptb1
            elif args.pr_reg_style == 'ptb2':
                self.w_lookup = nn.Parameter(torch.randn(self.vocab_size, self.tagset_size))
                self.posterior_reg_term = self.posterior_reg_term_ptb2
            elif args.pr_reg_style == 'ptb3':
                self.w_lookup = nn.Linear(self.embedding_dim, self.tagset_size)
                self.posterior_reg_term = self.posterior_reg_term_ptb3
            elif args.pr_reg_style == 'ptb4':
                pre_train_emb = 1024
                self.w_lookup = nn.Linear(pre_train_emb, self.tagset_size)
                self.posterior_reg_term = self.posterior_reg_term_ptb4
            else:
                print('unknow posterior reg style')
                sys.stdout.flush()
        else:
    ...
    def get_crf_beta_score(self, condi):
        x = condi['inps']
        ...
        # logp_Vocab = self.q_binary(states.view(-1, self.hidden_dim))[1:]  # L+1*bsz*sl x K
        logp_Vocab = self.q_binary(states)[1:]  # L+1*bsz*sl x K

        logp_Vocab = logp_Vocab.view(self.L, bsz, seqlen, self.tagset_size).transpose(1,2)
        return logp_Vocab

    def get_gen_beta_hmm(self, condi):
        '''
            :return: emission scores.
        '''
        x = condi['inps']
        bsz, seqlen = x.size()
        z_emb = condi['z_emb'].weight.narrow(0, 0, self.tagset_size)
        x_emb = condi['x_emb'].weight

        # 1. use a softmax to parametrize this locally normalized model.
        if self.decoder_type == 'gen1':
            emission = self.emission_lookup(z_emb) #state_size * vocab_size


        # 2. use a dot product result.
        if self.decoder_type == 'gen2':
            temp = self.emission_linear(z_emb) # state_size * embedding_dim
            emission = torch.mm(temp, x_emb.t())
            emission = torch.log_softmax(emission, dim=1)

        # gather the emission.
        logP = emission[:, x.view(-1)]
        result = logP.reshape(-1, bsz, seqlen)

        return result.permute(2, 1, 0).unsqueeze(0) # 1 x seqlen x bsz x K

    def get_gen_trans_hmm(self, condi):
        x = condi['inps']
        bsz, seqlen = x.size()
        K = self.tagset_size
        ...
        # trans_lps = torch.log_softmax(tscores, dim=1)
        # init_lps = torch.log_softmax(self.init_lin, dim=0)
        trans_lps = tscores.unsqueeze(0).expand(bsz, K, K)
        trans_lps = torch.log_softmax(trans_lps.contiguous().view(-1, K), dim=1)
        trans_lps = trans_lps.view(bsz, K, K)
        init_lps = torch.log_softmax(self.init_lin, dim=0).expand(bsz, -1)  # bsz x K
    ...
    def get_gen_beta_score(self, condi):
        x = condi['inps']
        combotargs = condi['combo_targs']
        # first form a matrix of size L * seqlen * bsz * embedding to denote the embeddings of L words.
        bsz, seqlen = x.size()
    ...
    def get_lstm_span_scores(self, condi):
        ...
        x = condi['inps'].transpose(0,1)
        seqlen, bsz_orig = x.shape
        layers, rnn_size = self.layers, self.hidden_dim
        inpembs = condi['tgt'].transpose(0,1)

        x_vec = torch.cat([self.bos_emb.expand(-1, bsz_orig, -1), inpembs, self.eos_emb.expand(-1, bsz_orig, -1)],
                          dim=0)
        q_h, _ = self.lstm(x_vec, None)
    ...
    def get_transition_score(self, uniqenc, seqlen, bsz):
        """
        args:
        ...
        trans_lps = torch.log_softmax(trans_lps.contiguous().view(-1, K), dim=1)
        trans_lps = trans_lps.view(bsz, K, K)
            # nn.LogSoftmax(dim=1)(trans_lps.view(-1, K)).view(bsz, K, K)
    ...
    def posterior_reg_term_ptb1(self, sent, condi, expected_count):
        '''
            gather the distinct field, and their related position in the sentence.
            condi['src']: bsz x  seqlen  x  3, where the first dim is field name, the second field is the index and
            the last term is  the word used.
            ...
            expected_count:  L x  seqlen  x  bsz x K
        '''
        # idea 1. We can directly regularize based on the dictionary.

        # idea 2. We can regularize so that the total number of clusters per sentence is a fixed number.
        ...
        if True:
            ...
            bsz = pr_mask.size(2)
            ''' get the normal penalizing counts'''
            reward_term = (pr_mask * expected_count[0])

            return -reward_term.sum() / bsz


    def posterior_reg_term_ptb2(self, sent, condi, expected_count):
        '''
            gather the distinct field, and their related position in the sentence.
            condi['src']: bsz x  seqlen  x  3, where the first dim is field name, the second field is the index and
            the last term is  the word used.
        ...
        yy = condi['targs']
        bsz, seqlen = yy.shape
        yy_tag = torch.log_softmax(self.w_lookup[yy.transpose(0,1)], dim=-1) # seqlen x bsz x K

        # mini = -torch.sum(expected_count[0][0] * yy_tag, dim=-1) # seqlen x bsz
        mini = -torch.sum(expected_count[0][0].log() * yy_tag.exp(), dim=-1)


        state_count = expected_count[0][0].sum(0) # bsz x K
        normalize = state_count.sum(1).unsqueeze(1).expand(-1, self.tagset_size)
        state_prob = state_count / (normalize + 1e-15)
        # regularize the entropy of this, hope it's large.
        maxi = -torch.sum(state_prob * state_prob.log(), dim=1)
        maxi = maxi.sum()

        # regularize the w matrix to be sparse by regularizing the entropy.
        w_entr = -torch.sum(yy_tag.exp() * yy_tag, dim=-1) # seqlen x bsz  hope to make this small.

        return (mini.sum() - maxi + w_entr.sum()) / bsz

    def posterior_reg_term_ptb3(self, sent, condi, expected_count):
        '''
            gather the distinct field, and their related position in the sentence.
            condi['src']: bsz x  seqlen  x  3, where the first dim is field name, the second field is the index and
            the last term is  the word used.
        ...
        # mini = -torch.sum(expected_count[0][0] * yy_tag, dim=-1) # seqlen x bsz
        mini = -torch.sum((expected_count[0][0] + 1e-15).log() * yy_prob, dim=-1)
        ...
        # regularize the w matrix to be sparse by regularizing the entropy.
        w_entr = -torch.sum(yy_prob * yy_tag, dim=-1) # seqlen x bsz  hope to make this small.

        # print(yy_tag.mean().item(), yy_prob.mean().item(), expected_count[0][0].sum().item(), w_entr.mean().item(),
        #       maxi.item(), mini.sum().item())

        return (mini.sum() - maxi + w_entr.sum()) / bsz


    def posterior_reg_term_ptb4(self, sent, condi, expected_count):
        '''
            gather the distinct field, and their related position in the sentence.
            condi['src']: bsz x  seqlen  x  3, where the first dim is field name, the second field is the index and
            the last term is  the word used.
        ...
        yy_emb = condi['tgt'].transpose(0,1)
        bsz, seqlen = yy.shape
        yy_tag = torch.log_softmax(self.w_lookup(yy_emb), dim=-1) # seqlen x bsz x K
        # print(yy_tag.shape)
        # print(expected_count[0][0].shape)
        ...
        # regularize expected counts to be diverse within the sentence.
        # make sure the sentence have x diverse tags.
        ...
        # mask_count = (state_count >= 1).sum(-1) >  0.7 * seqlen # bsz
        # #############################
        # filtered = state_count[mask_count]
        # maxi = filtered[filtered < 1].sum()  # hope to make larger by l1 norm.

        # print(state_count.shape)
        # print(mask_count.shape)
        # mask_count.unsqueeze(1).expand(bsz, self.tagset_size) * state_count[state_count < 1]
        ...
        # torch.log(expected_count)

        return (mini.sum() - maxi + w_entr.sum()) / bsz

    def posterior_reg_term_toy(self, sent, condi, expected_count):
        '''
            gather the distinct field, and their related position in the sentence.
            condi['src']: bsz x  seqlen  x  3, where the first dim is field name, the second field is the index and
            the last term is  the word used.
        ...
        if condi is None:
            penality = (expected_count[0].sum(3).sum(1).sum(0))
            # print(penality)
            penality = ((penality - 5) * (penality - 5)).mean()
    ...
    def posterior_reg_term_phraseStyled(self, sent, condi, expected_count):
        ...
        return (pen_term.sum() +  count_pen) / bsz - reward_term
    ...
    def get_weights(self, sent, condi):
        ...
        if self.decoder_type == 'gen':
            condi['combo_targs'] = self.make_combo_tags(condi['targs'], self.L)
            scores_beta = self.get_gen_beta_score(condi)
            init, trans = self.get_trans(None, sent.size()[1], sent.size()[0])
        elif self.decoder_type == 'gen1' or  self.decoder_type == 'gen2':
            scores_beta = self.get_gen_beta_hmm(condi)
            init, trans = self.get_gen_trans_hmm(condi)
        elif self.decoder_type == 'crf':
            scores_temp = self.get_span_scores(condi)
            scores_beta = self.get_emission_score(scores_temp)
            init, trans = self.get_trans(None, sent.size()[1], sent.size()[0])
        else:
            scores_beta = self.get_crf_beta_score(condi)
